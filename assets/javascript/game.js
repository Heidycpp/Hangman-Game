...
"poltergeist"
		...
		showloses = document.getElementById("loses");
		showGuessCheck = document.getElementById("letterCheck");
		loseAudio = new Audio('assets/audio/michael.mp3');
		winAudio = new Audio('assets/audio/psycho.mp3');
	...
	document.onkeyup = function(event) {
		...
		if(game.guessed.indexOf(guess) != -1){
			...
			if (game.solution.indexOf(guess) == -1){
				...
				if(game.lives < 1){
					showStatus.innerHTML = "You lose! Game over.";
				...
				showLives.innerHTML = game.lives;
				showGuessCheck.innerHTML = "<span class='text-success'>" + guess + " found!</span>";
				...
				if(game.reveal.indexOf("_") == -1){
					showStatus.innerHTML = "You got it! You know your Horror Films.";
					...
					if(game.lives == 6){
						showStatus.innerHTML = "PERFECT SCORE!";
					}
...
window.onLoad = horrorFilmsHangman();
